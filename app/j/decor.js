$(document).ready(function(){
	...
	if ($(".footer").length && $(".side").length) {
		function layout(){
			var delta = $(window).scrollTop() + $(window).height() - $(".footer").offset().top,
				...
				iHeight = $(window).height() - 170 - shift,
				iMinHeight = $('.side__i').height() + 50;
			
			$(".side").css({
				height: iHeight,
				'min-height': iMinHeight
			});
			$('.section').css({
				'min-height': iMinHeight 
			});
	...
	// servnote
	$('.servnote__close').click(function() {
		$(this).parent().slideUp();
	});
	...
	$(".sidenav").each(function(){
		var o = $(this),
			els = $("LI", this),
			em = $(".sidenav__em, .sidenav__em-text-oveflow", this);	
				...
				if (o.hasClass("idrop_open")) {
					...
					if (!o.hasClass("idrop_select")) {
						drop.css({left:0,top:0}).show().css({
							left:head.offset().left + head.outerWidth() - 111 - drop.offset().left,
	...
	$(".player").each(function(){
		...
		$(".player__track__i", this).each(function(){
	...
	// setup__step
	$("body").delegate(".setup__step__bt-next", "click", function(){
		var par = $(this).closest(".setup"),
			steps = $(".setup__step"),
			steps_cur = steps.filter(".setup__step_cur");
			
		if (steps_cur.nextAll().length) {
			steps_cur.removeClass("setup__step_cur");
			steps_cur.next().addClass("setup__step_cur");
		}
		return false;
	});
